...
myPutStr :: String -> IO ()
myPutStr = sequence_ . map putChar
...
fillEquiTri :: Window -> Int -> Int -> Int -> Color -> Bool -> IO ()
...
fillStarOfDavid :: Window -> Int -> Int -> Int -> Color -> IO ()
...
snowflake w x y size = helper w x y size (nextColor Green)
    where nextColor       :: Color -> Color
          ...
          helper :: Window -> Int -> Int -> Int -> Color -> IO ()
